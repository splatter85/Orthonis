...
The OFC4 authoring container had no .NET SDK, so no local C# execution was claimed at that boundary. Hosted Windows evidence was not owner-PC acceptance. OFC5 later ran the existing bounded adapter on one explicitly selected owner PC; empty or incomplete results still never certify PC health.
